...
A System only stores Attributes when it needs them which means that the System does not store any unnecessary data and thus saves memory. 
...
The Visual Effect Graph only stores an Attribute that you write to in simulation data if a later [Context](Contexts.md) uses the attribute. This means that:
